...
export default function HeritageNavbar({ 
  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 px-4 sm:px-8 py-3.5 sm:py-4 transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between px-4 sm:px-6 py-2 sm:py-2.5 rounded-full border border-[#c5a059]/35 bg-[#160f0a]/80 backdrop-blur-md shadow-[0_4px_25px_rgba(0,0,0,0.65)]">
        ...
        <div className="flex items-center gap-4 sm:gap-7">
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`group relative py-1 cursor-pointer flex items-center gap-1.5 transition-colors duration-300 ${
                    isActive 
                ...
                >
                  ...
                  <span
                    className={`absolute bottom-0 left-1/2 -translate-x-1/2 h-[1px] bg-gradient-to-r from-transparent via-[#d4af37] to-transparent shadow-[0_0_8px_rgba(212,175,55,0.5)] transition-all duration-300 ease-out pointer-events-none ${
                      isActive
                        ? 'w-full opacity-100'
          ...
          <button 
            ...
            className={`group relative flex items-center gap-1.5 px-3.5 sm:px-4 py-1.5 rounded-full border text-xs font-marcellus tracking-wider transition-all duration-300 cursor-pointer ${
              currentView === 'auth'
          ...
          {navItems.map((item) => {
            ...
            return (
              <button
                key={item.id}
                onClick={() => handleNavClick(item.id)}
                className={`py-2 text-left font-marcellus text-sm tracking-widest flex items-center gap-2.5 transition-colors cursor-pointer ${
                  isActive ? 'text-[#ffd27d] font-bold' : 'text-[#d8c4a4] hover:text-[#fff2d1]'
                }`}
              >
